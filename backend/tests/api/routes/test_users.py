...
def test_get_existing_user_as_superuser(
    ...
    user = create_test_user(session=db, user_create=user_in)
    user_id = user.id
    r = client.get(
...
def test_create_user_existing_username(
    ...
    password = random_lower_string()
    user_in = UserCreate(email=username, password=password)
    create_test_user(session=db, user_create=user_in)
    data = {"email": username, "password": password}
    r = client.post(
        f"{settings.API_V1_STR}/users/",
        headers=superuser_token_headers,
...
def test_retrieve_users(
    ...
    create_test_user(session=db, user_create=user_in2)
...
def test_update_user_email_exists(
    ...
    password2 = random_lower_string()
    user_in2 = UserCreate(email=username2, password=password2)
    user2 = create_test_user(session=db, user_create=user_in2)
